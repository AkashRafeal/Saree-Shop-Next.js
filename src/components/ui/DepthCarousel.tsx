...
const DEFAULT_ITEMS: DepthCarouselItem[] = [
  { image: 'https://images.unsplash.com/photo-1610030469983-98e550d6193c?auto=format&fit=crop&w=800&q=85', alt: 'Kanchipuram Silk' },
  { image: 'https://images.unsplash.com/photo-1617627143750-d86bc21e42bb?auto=format&fit=crop&w=800&q=85', alt: 'Banarasi Brocade' },
  { image: 'https://images.unsplash.com/photo-1583391733956-3750e0ff4e8b?auto=format&fit=crop&w=800&q=85', alt: 'Bridal Heritage' },
  { image: 'https://images.unsplash.com/photo-1566174053879-31528523f8ae?auto=format&fit=crop&w=800&q=85', alt: 'Chanderi & Organza' },
  { image: 'https://images.unsplash.com/photo-1609357605129-26f69add5d6e?auto=format&fit=crop&w=800&q=85', alt: 'Tussar Wild Silk' },
  { image: '/images/teal_ethnic_suit_embroidered.jpg', alt: 'Cotton & Linen' },
];
...
export const DepthCarousel: React.FC<DepthCarouselProps> = ({
  ...
  const tweenTo = useCallback(
    (target: number, animate: boolean) => {
      ...
      const dur = animate ? (cfg.duration || 700) / 1000 : 0;
  ...
  const handleArrowPress = useCallback(
    (step: number) => {
      const now = performance.now();
      // Block any bounce, duplicate listener, or rapid triggers within 450ms
      if (now - lastNavTimeRef.current < 450) {
        return;
      }
      lastNavTimeRef.current = now;
      resetAutoplayTimer();
      navigateByRef.current(step);
    },
    [resetAutoplayTimer]
  );

  // Single global keyboard listener without duplicate events or key-repeats
  useEffect(() => {
    const handleGlobalKeyDown = (e: KeyboardEvent) => {
      // Prevent OS key-repeat from skipping multiple cards
      if (e.repeat) return;
          ...
          >
            <img
              ...
              onError={(e) => {
                ...
                if (!target.src.includes('photo-1566174053879-31528523f8ae')) {
                  target.src = 'https://images.unsplash.com/photo-1566174053879-31528523f8ae?auto=format&fit=crop&w=800&q=85';
                }
              }}
            />
            <span
